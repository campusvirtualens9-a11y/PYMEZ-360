...
export default function CollectionsPage() {
  ...
  const overdueCount = receivables.filter(r => r.due_date && new Date(r.due_date + 'T00:00:00') < new Date(new Date().toDateString())).length
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <table className="w-full text-sm min-w-[600px]">
              ...
              <tbody>
                {filtered.map((r) => {
                  const isOverdue = r.due_date && new Date(r.due_date + 'T00:00:00') < new Date(new Date().toDateString())
                  return (
                  <tr key={r.id} className={`border-b border-slate-50 hover:bg-slate-50 ${isOverdue ? 'bg-red-50/40' : ''}`}>
                    <td className="px-5 py-3 font-medium text-slate-800">{r.customer?.name ?? '—'}</td>
                    <td className="px-5 py-3 text-slate-500 text-xs">
                      {r.sale?.date ? <span>Venta del {formatDate(r.sale.date)}</span> : '—'}
                    </td>
                    ...
                    <td className="px-5 py-3">{dueBadge(r.due_date)}</td>
